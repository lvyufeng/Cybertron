...
from cybertron.transformers import BertConfig, is_ms_available
...
from ...test_modeling_ms_common import ModelTesterMixin, floats_tensor, ids_tensor, random_attention_mask
...
if is_ms_available():
    ...
    from cybertron.transformers.models.bert.modeling_bert import BERT_PRETRAINED_MODEL_ARCHIVE_LIST
...
class BertModelTester:
    ...
    def create_and_check_for_multiple_choice(
        ...
        model.set_train(False)
        multiple_choice_inputs_ids = input_ids.unsqueeze(1).expand(mindspore.Tensor((-1, self.num_choices, -1)))
        multiple_choice_token_type_ids = token_type_ids.unsqueeze(1).expand(mindspore.Tensor((-1, self.num_choices, -1)))
        multiple_choice_input_mask = input_mask.unsqueeze(1).expand(mindspore.Tensor((-1, self.num_choices, -1)))
        result = model(
...
class BertModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (BertLMHeadModel,) if is_ms_available() else ()
    fx_compatible = True
