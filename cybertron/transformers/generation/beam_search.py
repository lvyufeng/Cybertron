...
PROCESS_INPUTS_DOCSTRING = r"""
    ...
    Return:
        `UserDict`: A dictionary composed of the fields as defined above:
            ...
            - **next_beam_scores** (`mindspore.Tensor` of shape `(batch_size * num_beams)`) -- Updated scores of all
              non-finished beams.
            - **next_beam_tokens** (`mindspore.Tensor` of shape `(batch_size * num_beams)`) -- Next tokens to be added
...
FINALIZE_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`mindspore.Tensor` of shape `(batch_size * num_beams, sequence_length)`):
            ...
            [What are input IDs?](../glossary#input-ids)
        final_beam_scores (`mindspore.Tensor` of shape `(batch_size * num_beams)`):
            The final scores of all non-finished beams.
        final_beam_tokens (`mindspore.Tensor` of shape `(batch_size * num_beams)`):
            The last tokens to be added to the non-finished beam_hypotheses.
        final_beam_indices (`mindspore.Tensor` of shape `(batch_size * num_beams)`):
    ...
    Return:
        `mindspore.Tensor` of shape `(batch_size * num_return_sequences, sequence_length)`: The generated sequences.
...
class BeamSearchScorer(BeamScorer):
    ...
    def finalize(
        ...
        decoded = ops.zeros((batch_size * self.num_beam_hyps_to_keep, sent_max_len), input_ids.dtype)
...
class ConstrainedBeamSearchScorer(BeamScorer):
    ...
    def step_sentence_constraint(
        ...
        track_new = {
            "new_seqs": full_hypotheses.asnumpy().tolist(),
            "new_states": [],
            "new_indices": [],
            "new_tokens": [],
            "new_scores": [],
        ...
        for seq_idx, pre_seq in enumerate(this_batch_input_ids):
            ...
            if not advance_state.completed:
                advance_tokens = mindspore.Tensor(advance_state.advance(), mindspore.int32)
        ...
        if len(track_new["new_indices"]) > 0:
            ...
            indices = ops.Sort(descending=True)(zipped)[1]
