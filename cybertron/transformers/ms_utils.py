...
parsed_ms_version_base = version.parse(version.parse(mindspore.__version__).base_version)



def ms_int_div(tensor1, tensor2):
    """
    A function that performs integer division across different versions of PyTorch.
    """
    return ops.div(tensor1, tensor2, rounding_mode="floor")
...
def prune_linear_layer(layer: nn.Dense, index: mindspore.Tensor, dim: int = 0) -> nn.Dense:
    """
    ...
    new_layer = nn.Dense(new_size[1], new_size[0], has_bias=layer.has_bias)
...
def find_pruneable_heads_and_indices(
    ...
    index = ops.masked_select(ops.arange(len(mask)), mask).astype(mindspore.int32)
    return heads, index
