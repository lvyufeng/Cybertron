...
from ..ms_utils import ms_int_div
...
class BeamSearchDecoderOnlyOutput(ModelOutput):
    ...
    Args:
        ...
        beam_indices (`tuple(tuple(mindspore.Tensor))`, *optional*, returned when `output_scores=True` is passed or when `config.output_scores=True`):
...
class GenerationMixin:
    ...
    def _prepare_decoder_input_ids_for_generation(
        ...
        if model_kwargs is not None and "decoder_input_ids" in model_kwargs:
            ...
            return ops.ones((batch_size, 1), dtype=mindspore.int32) * decoder_start_token_id
    ...
    def _update_model_kwargs_for_generation(
        ...
        if "token_type_ids" in model_kwargs:
            ...
            model_kwargs["token_type_ids"] = ops.cat([token_type_ids, token_type_ids[:, -1].expand_dims(-1)], axis=-1)
    ...
    def compute_transition_scores(
        ...
        Parameters:
            ...
            scores (`tuple(mindspore.Tensor)`):
                ...
                `mindspore.Tensor` with up to `max_new_tokens` elements (one element for each generated token), with
                each tensor of shape `(batch_size*num_beams, config.vocab_size)`.
            beam_indices (`tuple(tuple(mindspore.Tensor))`, *optional*):
                Beam indices of generated token id at each generation step. `mindspore.Tensor` of shape
        ...
        **kwargs,
        ...
        if not self.config.is_encoder_decoder:
            if (
                ...
                and ops.sum((inputs_tensor[:, -1] == generation_config.pad_token_id).astype(mindspore.int32)) > 0
            ):
                logger.warning(
    ...
    def contrastive_search(
        ...
        **model_kwargs,
        ...
        unfinished_sequences = ops.ones(input_ids.shape[0])
        ...
        while True:
            if synced_gpus:
                ...
                this_peer_finished_flag = mindspore.Tensor(0.0 if this_peer_finished else 1.0).to(input_ids.device)
            ...
            top_k_probs, top_k_ids = ops.top_k(next_probs, k=top_k)
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # argmax
            next_tokens = ops.argmax(next_tokens_scores, axis=-1).astype(mindspore.int32)
        ...
        **model_kwargs,
        ...
        while True:
            if synced_gpus:
                ...
                # dist.all_reduce(this_peer_finished_flag, op=dist.ReduceOp.SUM)
                # did all peers finish? the reduced sum will be 0.0 then
                if this_peer_finished_flag.asnumpy().item() == 0.0:
                    break
            ...
            # next_token_scores, next_tokens = ops.top_k(
            #     next_token_scores, 2 * num_beams, dim=1, largest=True, sorted=True
            # )
            next_token_scores, next_tokens = ops.Sort(1, descending=True)(next_token_scores)
            next_token_scores = next_token_scores[:2 * num_beams]
            next_tokens = next_tokens[:2 * num_beams]

            next_indices = ms_int_div(next_tokens, vocab_size)
            ...
            input_ids = ops.cat([input_ids[beam_idx, :], beam_next_tokens.expand_dims(-1)], axis=-1)
    ...
    def beam_sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            next_tokens = ops.multinomial(probs, num_sample=2 * num_beams)
            next_token_scores = ops.gather_elements(next_token_scores, -1, next_tokens)

            next_token_scores, _indices = ops.Sort(axis=1, descending=True)(next_token_scores)
    ...
    def group_beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            reordering_indices = ops.zeros(batch_size * num_beams, dtype=mindspore.int32)
            ...
            for beam_group_idx in range(num_beam_groups):
                ...
                reordering_indices[batch_group_indices] = (
                    num_beams * ms_int_div(beam_idx, group_size) + group_start_idx + (beam_idx % group_size)
                )
...
def _ranking_fast(
    ...
    row in the batch.
    """
    norm_context_hidden = context_hidden / ops.norm(context_hidden, dim=2, keepdim=True)
    norm_next_hidden = next_hidden / ops.norm(next_hidden, dim=2, keepdim=True)
    ...
    selected_idx = contrastive_score.argmax(axis=-1)  # [B]
    return selected_idx