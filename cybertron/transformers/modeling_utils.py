...
def get_parameter_dtype(parameter: Union[nn.Cell, GenerationMixin, "ModuleUtilsMixin"]):
    ...
    """
    last_dtype = None
    for t in parameter.trainable_params():
        last_dtype = t.dtype
...
def load_sharded_checkpoint(model, folder, strict=True):
    ...
    Args:
        model (`torch.nn.Cell`): The model in which to load the checkpoint.
    ...
    return torch.nn.Cells.module._IncompatibleKeys(missing_keys, unexpected_keys)
...
def load_state_dict(checkpoint_file: Union[str, os.PathLike]):
    ...
    try:
        return mindspore.load_checkpoint(checkpoint_file)
...
def _load_state_dict_into_model(model_to_load, state_dict, start_prefix):
    ...
    def load(module: nn.Cell, state_dict, prefix=""):
        ...
        if len([key for key in state_dict if key.startswith(prefix)]) > 0:
            ...
            # else:
            # module._load_from_state_dict(*args)
            mindspore.load_param_into_net(module, state_dict)
...
class ModuleUtilsMixin:
    """
    A few utilities for `torch.nn.Cells`, to be used as a mixin.
    """
    ...
    def invert_attention_mask(self, encoder_attention_mask: Tensor) -> Tensor:
        ...
        encoder_extended_attention_mask = (1.0 - encoder_extended_attention_mask) * Tensor(np.finfo(mindspore.dtype_to_nptype(self.dtype)).min, self.dtype)
    ...
    def get_extended_attention_mask(
        self, attention_mask: Tensor, input_shape: Tuple[int], dtype: mindspore.float_ = None) -> Tensor:
        """
        ...
        extended_attention_mask = (1.0 - extended_attention_mask) * 1e-10
        return extended_attention_mask
...
class PreTrainedModel(nn.Cell, ModuleUtilsMixin, GenerationMixin):
    ...
    @classmethod
    def _set_default_torch_dtype(cls, dtype: mindspore.dtype) -> mindspore.dtype:
        """
    ...
    def base_model(self) -> nn.Cell:
        """
        `torch.nn.Cell`: The main body of the model.
        """
                ...
                for name, module in decoder_modules.items():
                    if name.isdigit():
                        ...
                        raise ValueError(
                            ...
                            " a circular dependency between two or more `nn.Cells` of your model."
                        )
                    else:
    ...
    def _get_resized_lm_head(
        ...
        Args:
            ...
            new_num_tokens (`int`, *optional*):
                ...
                Increasing the size will add newly initialized vectors at the end. Reducing the size will remove
                vectors from the end. If not provided or `None`, just returns a pointer to the input tokens
                `torch.nn.Dense` module of the model without doing anything. transposed (`bool`, *optional*, defaults
                to `False`): Whether `old_lm_head` is transposed or not. If True `old_lm_head.size()` is `lm_head_dim,
                vocab_size` else `vocab_size, lm_head_dim`.

        Return:
            `torch.nn.Dense`: Pointer to the resized Linear Module or the old Linear Module if `new_num_tokens` is
            `None`
        """
        if new_num_tokens is None:
            return old_lm_head
        ...
        **kwargs,
        ...
        for shard_file, shard in shards.items():
            if safe_serialization:
                ...
                shard_list = [{"name": k, "data": v} for k, v in shard.items()]
                save_function(shard_list, os.path.join(save_directory, shard_file))
        ...
        * `low_cpu_mem_usage` algorithm:
        ...
        if pretrained_model_name_or_path is not None:
            ...
            if is_local:
                ...
                #     is_sharded = True
                if os.path.isfile(os.path.join(pretrained_model_name_or_path, subfolder, WEIGHTS_NAME)):
        ...
        if keep_in_fp32_modules is not None:
            for name, param in model.parameters_and_names():
                if any(module_to_keep_in_fp32 in name for module_to_keep_in_fp32 in keep_in_fp32_modules):
                    param = param.astype(mindspore.float32)
...
class PoolerAnswerClass(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.dense_0 = nn.Dense(config.hidden_size * 2, config.hidden_size)
        self.activation = nn.Tanh()
        self.dense_1 = nn.Dense(config.hidden_size, 1, bias=False)
...
def unwrap_model(model: nn.Cell) -> nn.Cell:
    """
    Recursively unwraps a model from potential containers (as used in distributed training).

    Args:
        model (`torch.nn.Cell`): The model to unwrap.
    """
