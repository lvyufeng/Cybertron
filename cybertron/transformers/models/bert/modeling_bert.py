...
"""MindSpore BERT model ported from Transformers."""
...
from cybertron.common.layers import Linear, Dropout
...
class BertEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.position_ids = Parameter(Tensor(np.arange(config.max_position_embeddings).reshape((1, -1))), 'position_ids')
        self.token_type_ids = Parameter(Tensor(np.zeros(self.position_ids.shape), dtype=mindspore.int32), 'token_type_ids')
...
class BertSelfAttention(nn.Cell):
    ...
    def construct(
        ...
        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            ...
            if use_cache:
                position_ids_l = mindspore.Tensor(key_length - 1, dtype=mindspore.int32).view(-1, 1)
            else:
            ...
            if self.position_embedding_type == "relative_key":
                relative_position_scores = bhld_lrd_to_bhlr(query_layer, positional_embedding)
                attention_scores = attention_scores + relative_position_scores
            elif self.position_embedding_type == "relative_key_query":
                relative_position_scores_query = bhld_lrd_to_bhlr(query_layer, positional_embedding)
                relative_position_scores_key = bhrd_lrd_to_bhlr(key_layer, positional_embedding)
...
class BertAttention(nn.Cell):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self_attn = BertSelfAttention(config, position_embedding_type=position_embedding_type)
    ...
    def prune_heads(self, heads):
        ...
        # Prune linear layers
        self.self_attn.query = prune_linear_layer(self.self_attn.query, index)
        self.self_attn.key = prune_linear_layer(self.self_attn.key, index)
        self.self_attn.value = prune_linear_layer(self.self_attn.value, index)
...
class BertLMPredictionHead(nn.Cell):
    def __init__(self, config):
        ...
        self.bias = Parameter(Tensor(np.zeros(config.vocab_size)), 'bias')
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Dense):
            ...
            module.weight.set_data(initializer(Normal(self.config.initializer_range), module.weight.shape))
            if module.bias is not None:
                module.bias.set_data(initializer('zeros', module.bias.shape))
        elif isinstance(module, nn.Embedding):
            init_tensor = initializer(Normal(self.config.initializer_range), module.embedding_table.shape)
...
class BertModel(BertPreTrainedModel):
    ...
    def construct(
        ...
        if token_type_ids is None:
            if self.embeddings.token_type_ids is not None:
                buffered_token_type_ids = self.embeddings.token_type_ids[:, :seq_length]
                buffered_token_type_ids_expanded = buffered_token_type_ids.expand(Tensor((batch_size, seq_length)))
                token_type_ids = buffered_token_type_ids_expanded
            else:
                token_type_ids = ops.zeros(input_shape, dtype=mindspore.int32)
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        if attention_mask is None:
            attention_mask = ops.ones(input_ids.shape, input_ids.dtype)
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, input_ids, attention_mask=None, **model_kwargs):
        ...
        attention_mask = ops.cat([attention_mask, ops.zeros((attention_mask.shape[0], 1), attention_mask.dtype)], axis=-1)
        dummy_token = ops.full(
            (effective_batch_size, 1), self.config.pad_token_id, dtype=mindspore.int32
        )
    ...
    def construct(
        ...
        if labels is not None:
            if self.config.problem_type is None:
                ...
                elif self.num_labels > 1 and (labels.dtype == mindspore.int32 or labels.dtype == mindspore.int32):
