...
from cybertron.transformers.configuration_utils import PretrainedConfig
from cybertron.transformers.testing_utils import TOKEN, USER, is_staging_test
...
class ConfigTester(object):
    ...
    def check_config_arguments_init(self):
        ...
        for key, value in config_common_kwargs.items():
            if key == "ms_dtype":
                if not is_ms_available():
                    continue
                else:
                    import mindspore

                    if config.ms_dtype != mindspore.float16:
                        wrong_values.append(("ms_dtype", config.ms_dtype, mindspore.float16))
