...
LOGITS_PROCESSOR_INPUTS_DOCSTRING = r"""
    ...
    Return:
        `mindspore.Tensor` of shape `(batch_size, config.vocab_size)`: The processed prediction scores.
...
class RepetitionPenaltyLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: mindspore.Tensor, scores: mindspore.Tensor) -> mindspore.Tensor:
        ...
        scores = ops.tensor_scatter_elements(scores, input_ids, score, 1)
        return scores
...
class TopPLogitsWarper(LogitsWarper):
    ...
    def __call__(self, input_ids: mindspore.Tensor, scores: mindspore.Tensor) -> mindspore.Tensor:
        sorted_logits, sorted_indices = ops.Sort(descending=False)(scores)
        ...
        indices_to_remove = ops.tensor_scatter_elements(sorted_indices_to_remove, sorted_indices, sorted_indices_to_remove, 1)
...
class TopKLogitsWarper(LogitsWarper):
    ...
    def __call__(self, input_ids: mindspore.Tensor, scores: mindspore.Tensor) -> mindspore.Tensor:
        ...
        indices_to_remove = scores < ops.top_k(scores, top_k)[0][..., -1, None]
        scores = scores.masked_fill(indices_to_remove, self.filter_value)
        return scores
...
def _get_generated_ngrams(banned_ngrams, prev_input_ids, ngram_size, cur_len):
    ...
    ngram_idx = tuple(prev_input_ids[start_idx:cur_len].asnumpy().tolist())
...
class NoBadWordsLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: mindspore.Tensor, scores: mindspore.Tensor) -> mindspore.Tensor:
        ...
        dynamic_banned_tokens = self._calc_banned_bad_words_ids(input_ids.asnumpy().tolist())
    ...
    def _calc_static_bad_word_mask(self, scores: mindspore.Tensor) -> mindspore.Tensor:
        ...
        return static_bad_words_mask.expand_dims(0).astype(mindspore.bool_)
    ...
    def _set_scores_to_inf_for_banned_tokens(
        ...
        if not banned_mask_list and self.static_bad_words_mask is None:
            ...
            if banned_mask_list:
                ...
                banned_mask = (
                    mindspore.COOTensor(banned_mask, indices, scores.shape)
                    .to_dense()
                    .bool()
                )
...
class InfNanRemoveLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: mindspore.Tensor, scores: mindspore.Tensor) -> mindspore.Tensor:
        ...
        scores[scores == float("inf")] = mindspore.Tensor(np.finfo(scores.asnumpy().dtype).max, scores.dtype)
